...
`TourApiClient` calls the external Korean TourAPI via `RestClient`. It requires `tour.service-key`; without it, requests throw `TOUR_API_NOT_CONFIGURED`. Responses are JSON-parsed defensively (single object vs. array `item` nodes both handled). `TourService` results are cached with Caffeine (`TourCacheConfig`). **Pet-friendly places**: `petFriendly=true` on `GET /api/tour/contents` and `/contents/nearby` switches the same call to the KTO pet-travel service (`KorPetTourService2`, `TourApiClient.PET_SERVICE`), which shares `contentId`, `areaCode`/`sigunguCode` and Korean `contentTypeId` codes with `KorService2`, so cart/trip flows are unchanged. `GET /api/tour/contents/{id}` always adds `petInfo` from `detailPetTour2` (null when the place is not registered or the pet service fails). Korean only. `CartItem.petFriendly` is set when a place is added from that detail and copied to `TripItem.petFriendly` (null = unknown, treated as not pet-friendly); `PetFriendlySummary` (`all`/`count`/`total`) rides on trip detail, public detail and feed cards, and `GET /api/feed?petFriendly=true` returns only trips whose every item is pet-friendly. Places added before this feature stay null until `APP_PET_BACKFILL_ON_STARTUP=true` runs `PetFriendlyBackfillService` once at boot (one KTO call per distinct contentId).

### Recommendations & preferences

`UserPreference` (managed by `preference/`) stores a user's `InterestTag`s, `PreferredRegion`s, `BudgetLevel`, and `TravelStyle` (enums in `entity/Enum/`). `RecommendationService` (`tour/recommendation/`) is **rule-based, not ML**: `PreferenceMapper` is the single source of truth that maps each `InterestTag` → TourAPI params (`contentTypeId`/`lclsSystm` codes) and each `PreferredRegion` → area/lDong region codes, then it composes sections (a personalized section plus dedicated Food/Cafe/Festival sections) by querying `TourService`. When a user has no saved preference it returns `buildDefaultRecommendations()`. The `PET_FRIENDLY` interest tag is not mapped to TourAPI params; instead it adds a dedicated "반려동물과 함께" section fetched from the KTO pet-travel service, and the chatbot's `searchTourism` tool has a `petFriendly` flag that routes to the same service. When extending recommendations, edit the `PreferenceMapper` switch rather than scattering TourAPI codes through the service.
